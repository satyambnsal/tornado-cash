/**
 * Tornado Cash Main Application
 * Privacy mixer interface with deposit, withdraw, and history tabs
 */
...
import { extractDepositInfo } from "../services/tornadoContract";
import type { ProofData } from "../types/tornado";

type Tab = "deposit" | "withdraw" | "history";

export function TornadoCashApp() {
  const [activeTab, setActiveTab] = useState<Tab>("deposit");
  const [balance, setBalance] = useState<string>("0");

  // Get auth context from existing XION boilerplate
  const { abstractAccount, logout } = useContext(
    AuthContext
  ) as AuthContextProps;

  // Get signing client from existing hook
  ...
  useEffect(() => {
    if (client && abstractAccount) {
      loadBalance();
    }
  }, [client, abstractAccount]);

  const loadBalance = async () => {
    if (!client || !abstractAccount) return;

    try {
      const bal = await client.getBalance(abstractAccount.id, "uxion");
  ...
  const handleDeposit = async (
    ...
    if (!contractService || !abstractAccount) {
      ...
      <div className="ui-min-h-screen ui-flex ui-items-center ui-justify-center ui-bg-background">
        <div className="ui-text-center ui-p-8">
          ...
          <p className="ui-text-sm ui-text-muted-foreground">
            Please sign in using the login screen
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="ui-min-h-screen ui-bg-background">
      ...
      <Header
        ...
        userAddress={abstractAccount?.id || null}
        balance={balance}
        ...
        <div className="ui-max-w-2xl ui-mx-auto">
          {activeTab === "deposit" && (
            <DepositTab
              ...
              denomination={denomination}
              balance={balance}
              userAddress={abstractAccount.id}
              chainId={CHAIN_ID}
