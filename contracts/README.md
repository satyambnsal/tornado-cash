...
CosmWasm contract for Tornado Cash
...
```bash
RES=$(xiond tx wasm store ./artifacts/zk_sudoku.wasm \
  --chain-id "$CHAIN_ID" \
  --gas-adjustment 1.3 \
  --gas-prices 0.1uxion \
  --gas auto \
...
- zk sudoku circuit: `vkey name: sudoku`, `vkey id: 6`

Verification keys and artifacts are stored under `vkeys/` and `artifacts/` respectively.

```
 address: xion1h26yyjht8ch5gg9kpxgcfrxvz0j30yp728zfq6
  name: satyam_torn
  pubkey: '{"@type":"/cosmos.crypto.secp256k1.PubKey","key":"A1CMbRpyw+zJUL/PV0EWH1zcqN9Rk2aoiovLSs52KyR0"}'
  type: local
...
moral pond moral lumber spider unaware agree breeze leave jacket sketch essay used brain tell inherit essay antenna ability demise promote auction tone host
```

```
xiond q bank balances $SATYAM_TORN --node $RPC_URL
```

```
xiond tx bank send $SATYAM2 $SATYAM_TORN  5960000uxion --chain-id xion-testnet-2 \
--node $RPC_URL \
--from $SATYAM2 \
